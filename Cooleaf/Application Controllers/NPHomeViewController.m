...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
	...
	if (self) {
		self.title = NSLocalizedString(@"Home", @"Home list view title");
...
//		self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:btn];
		self.navigationItem.backBarButtonItem = [[UIBarButtonItem alloc]
												 initWithTitle:NSLocalizedString(@"Home", @"Home list view title")
...
{
	NPHomeCell *cell = [tableView dequeueReusableCellWithIdentifier:@"NPHomeCell"];
			...
			if (!_joinActions[eventId])
			{
				if (join)
				{
					_joinActions[eventId] = [[NPCooleafClient sharedClient] joinEventWithId:eventId completion:^(NSError *error) {
						...
						NPHomeCell *c = (NPHomeCell *)[tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:row+1 inSection:0]];
						if (c)
							[c closeDrawer];
		...
		cell.actionTapped = ^(NSNumber *eventId, BOOL join) {
			if (!_joinActions[eventId])
			{
				if (join)
				{
					_joinActions[eventId] = [[NPCooleafClient sharedClient] joinEventWithId:eventId completion:^(NSError *error) {
						[_joinActions removeObjectForKey:eventId];
						NPHomeCell *c = (NPHomeCell *)[tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:_myEvents.count + 2 + row inSection:0]];
						if (c)
							[c closeDrawer];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
	if (indexPath.row == 0)
		...
		return [NPHomeCell cellHeightForEvent:_myEvents[indexPath.row - 1]];
	else if (indexPath.row-1 == _myEvents.count)
	{
		return _otherBranchesHeader.frame.size.height;
	}
	else
		return [NPOtherHomeCell cellHeightForEvent:_otherEvents[indexPath.row - _myEvents.count - 2]];
}
